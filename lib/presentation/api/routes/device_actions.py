...
2. **A raw action** (`action_type`, one of `click`, `click_first_match`, `click_bounds`,
   `scroll_up`, `back`, `wait`, `dump_nodes`, `screenshot`, `ocr_extract`): runs against whatever
   is currently on screen. Pass `target_screen_id` (a `MapperScreen` id) to also get navigated
   there first, the same way as a mapped action; without it, nothing is navigated, this fires
   immediately against the live screen, useful right after a previous call already confirmed the
   position (via its `resulting_screen_id`), or for a purely exploratory read.
...
`ocr_extract` result); `wait` takes `params: {"seconds": float}`; the rest need no selector.
