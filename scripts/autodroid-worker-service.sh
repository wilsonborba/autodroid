...
usage() {
  cat <<'EOF'
Usage: scripts/autodroid-worker-service.sh <command>
...
case "${1:-}" in
  ...
  logs)
    ...
    ;;
  install)
    exec "$SCRIPT_DIR/install-autodroid-worker-service.sh"
    ;;
  uninstall)
    exec "$SCRIPT_DIR/uninstall-autodroid-worker-service.sh"
    ;;
  *)
    usage
    exit 1
